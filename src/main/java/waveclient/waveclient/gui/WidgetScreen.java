...
package waveclient.waveclient.gui;

import waveclient.waveclient.WaveClient;
import waveclient.waveclient.gui.renderer.GuiDebugRenderer;
...
import waveclient.waveclient.gui.widgets.WRoot;
...
import waveclient.waveclient.utils.misc.CursorStyle;
...
public abstract class WidgetScreen extends Screen {
    ...
    @Override
    protected void init() {
        WaveClient.EVENT_BUS.subscribe(this);
    ...
    public void renderCustom(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (config != null && config.waveGlass.get()) {
            double factor = config.waveGlassTint.get() / 100.0;

            Color topLeft = new Color(24, 102, 138, (int) Math.round(120 * factor));
            Color topRight = new Color(19, 78, 104, (int) Math.round(95 * factor));
            Color bottomRight = new Color(12, 34, 48, (int) Math.round(45 * factor));
            Color bottomLeft = new Color(11, 32, 45, (int) Math.round(62 * factor));
            RENDERER.quad(0, 0, getWindowWidth(), getWindowHeight(), topLeft, topRight, bottomRight, bottomLeft);
        }
