...
public class EntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        for (Direction direction : DirectionAccessor.wave$getHorizontal()) {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) mc.world).wave$getEntityLookup();
        ...
        if (entityLookup instanceof SimpleEntityLookup<Entity> simpleEntityLookup) {
            SectionedEntityCache<Entity> cache = ((SimpleEntityLookupAccessor) simpleEntityLookup).wave$getCache();
            LongSortedSet trackedPositions = ((SectionedEntityCacheAccessor) cache).wave$getTrackedPositions();
            Long2ObjectMap<EntityTrackingSection<Entity>> trackingSections = ((SectionedEntityCacheAccessor) cache).wave$getTrackingSections();
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((EntityTrackingSectionAccessor) entityTrackingSection).<Entity>wave$getCollection()) {
