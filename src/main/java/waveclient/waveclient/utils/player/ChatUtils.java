...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Text.empty()
            ...
            .append("[")
            .append(Text.literal("Wave").setStyle(Style.EMPTY.withColor(TextColor.fromRgb(WaveClient.ADDON.color.getPacked()))))
            .append("] ");
    }

    public static Text getWavePrefix() {
        return PREFIX;
    }

    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned from the supplier the default Wave prefix is used.
     */
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        ...
        mc.execute(() -> ((IChatHud) mc.inGameHud.getChatHud()).wave$add(message, finalId));
    }
    ...
    public static MutableText formatCoords(Vec3d pos) {
        ...
        if (BaritoneUtils.IS_AVAILABLE) {
            Style style = coordsText.getStyle().withFormatting(Formatting.BOLD)
                ...
                .withClickEvent(new WaveClickEvent(
