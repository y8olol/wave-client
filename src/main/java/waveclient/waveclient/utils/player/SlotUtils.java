...
import waveclient.waveclient.mixin.CreativeInventoryScreenAccessor;
import waveclient.waveclient.mixin.ItemGroupsAccessor;
import waveclient.waveclient.mixin.MountScreenHandlerAccessor;
...
public class SlotUtils {
     ...
     *
     * Slot id mapping reference for inventory protocol.
    ...
    private static int creativeInventory(int i) {
        if (CreativeInventoryScreenAccessor.wave$getSelectedTab() != Registries.ITEM_GROUP.get(ItemGroupsAccessor.wave$getInventory()))
            return -1;
    ...
    private static int horse(ScreenHandler handler, int i) {
        LivingEntity entity = ((MountScreenHandlerAccessor) handler).wave$getMount();
