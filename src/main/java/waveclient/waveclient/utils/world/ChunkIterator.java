...
import waveclient.waveclient.mixin.ClientChunkManagerAccessor;
import waveclient.waveclient.mixin.ClientChunkMapAccessor;
...
public class ChunkIterator implements Iterator<Chunk> {
    private final ClientChunkMapAccessor map = (ClientChunkMapAccessor) (Object) ((ClientChunkManagerAccessor) mc.world.getChunkManager()).wave$getChunks();
    ...
    private Chunk getNext() {
        ...
        while (i < map.wave$getChunks().length()) {
            chunk = map.wave$getChunks().get(i++);
