...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(WaveRenderPipelines.POST_IMAGE);
        WaveClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(WaveClient.identifier("textures/chams." + fileFormat));
            ...
            WaveClient.LOG.error("Error loading the chams shader", e);
        }
    }
    ...
    private static final DynamicUniformStorage<UniformData> UNIFORM_STORAGE = new DynamicUniformStorage<>("Wave - Image UBO", UNIFORM_SIZE, 16);
