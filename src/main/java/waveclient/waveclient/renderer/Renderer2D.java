...
import waveclient.waveclient.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean textured) {
        ...
        triangles = new MeshBuilder(textured ? WaveRenderPipelines.UI_TEXTURED : WaveRenderPipelines.UI_COLORED);
        lines = new MeshBuilder(WaveRenderPipelines.UI_COLORED_LINES);
    }
    ...
    public void render(String samplerName, GpuTextureView samplerView, GpuSampler sampler) {
        ...
        MeshRenderer.begin()
            ...
            .pipeline(WaveRenderPipelines.UI_COLORED_LINES)
            .mesh(lines)
            .end();

        MeshRenderer.begin()
            .attachments(MinecraftClient.getInstance().getFramebuffer())
            .pipeline(textured ? WaveRenderPipelines.UI_TEXTURED : WaveRenderPipelines.UI_COLORED)
            .mesh(triangles)
