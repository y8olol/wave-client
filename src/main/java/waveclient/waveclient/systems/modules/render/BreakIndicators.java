...
import waveclient.waveclient.mixin.WorldRendererAccessor;
...
import waveclient.waveclient.systems.modules.world.PacketMine;
...
public class BreakIndicators extends Module {
    ...
    private void renderNormal(Render3DEvent event) {
        Map<Integer, BlockBreakingInfo> blocks = ((WorldRendererAccessor) mc.worldRenderer).wave$getBlockBreakingInfos();

        float ownBreakingStage = ((ClientPlayerInteractionManagerAccessor) mc.interactionManager).wave$getBreakingProgress();
        BlockPos ownBreakingPos = ((ClientPlayerInteractionManagerAccessor) mc.interactionManager).wave$getCurrentBreakingBlockPos();
