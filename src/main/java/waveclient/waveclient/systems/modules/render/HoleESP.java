...
public class HoleESP extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockIterator.register(horizontalRadius.get(), verticalRadius.get(), (blockPos, blockState) -> {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (((AbstractBlockAccessor) block).wave$isCollidable() && !breakable) bedrock++;
    ...
    private boolean validHole(BlockPos pos) {
        ...
        if (((AbstractBlockAccessor) block).wave$isCollidable()) return false;

        for (int i = 0; i < holeHeight.get(); i++) {
            if (((AbstractBlockAccessor) chunk.getBlockState(pos.up(i)).getBlock()).wave$isCollidable()) return false;
        }
