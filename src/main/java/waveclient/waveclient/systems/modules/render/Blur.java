...
import waveclient.waveclient.events.render.RenderAfterWorldEvent;
import waveclient.waveclient.gui.WidgetScreen;
import waveclient.waveclient.renderer.FixedUniformStorage;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> waveUi = sgScreens.add(new BoolSetting.Builder()
        .name("wave-ui")
        .description("Applies blur to Wave screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        WaveClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        WaveClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        boolean waveGlass = config != null && config.waveGlass.get();
        boolean forceWaveGuiBlur = waveGlass && !isActive() && mc.currentScreen instanceof WidgetScreen;

        // Enable / disable with fading
        boolean shouldRender = forceWaveGuiBlur || shouldRender();
        ...
        // Update strength
        IntFloatImmutablePair strength = forceWaveGuiBlur
            ? strengths[Math.max(0, Math.min(strengths.length - 1, config.waveGlassBlur.get() - 1))]
            : strengths[(int) ((this.strength.get() - 1) * progress)];
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.getFramebuffer().getColorAttachmentView(), WaveRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], WaveRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], WaveRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.getFramebuffer())
            .pipeline(WaveRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return waveUi.get();
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("Wave - Blur UBO", UNIFORM_SIZE, 6);
