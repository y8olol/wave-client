...
import waveclient.waveclient.events.render.ApplyTransformationEvent;
import waveclient.waveclient.events.render.RenderItemEntityEvent;
import waveclient.waveclient.mixin.ItemRenderStateAccessor;
import waveclient.waveclient.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemRenderStateAccessor) event.renderState.itemRenderState).wave$getLayerCount(); i++) {
            ItemRenderState.LayerRenderState layer = ((ItemRenderStateAccessor) event.renderState.itemRenderState).wave$getLayers()[i];
            ModelInfo info = getInfo(layer.getQuads());

            matrices.push();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).wave$getTransform());
