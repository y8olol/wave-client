...
public class Anchor extends Module {
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (foundHole) {
            ...
            ((IVec3d) mc.player.getVelocity()).wave$set(deltaX, mc.player.getVelocity().y - (pull.get() ? pullSpeed.get() : 0), deltaZ);
        }
    }
    ...
    private boolean isAir(int x, int y, int z) {
        blockPos.set(x, y, z);
        return !((AbstractBlockAccessor)mc.world.getBlockState(blockPos).getBlock()).wave$isCollidable();
    }
}
