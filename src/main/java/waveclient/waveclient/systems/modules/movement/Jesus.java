...
import waveclient.waveclient.events.entity.player.CanWalkOnFluidEvent;
import waveclient.waveclient.events.entity.player.PlayerMoveEvent;
import waveclient.waveclient.events.packets.PacketEvent;
import waveclient.waveclient.events.world.CollisionShapeEvent;
import waveclient.waveclient.events.world.TickEvent;
import waveclient.waveclient.mixin.EntityMixin;
...
import waveclient.waveclient.systems.modules.movement.speed.modes.Strafe;
...
public class Jesus extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (bubbleColumn) {
            if (mc.options.jumpKey.isPressed() && movingEntity.getVelocity().getY() < 0.11) ((IVec3d) movingEntity.getVelocity()).wave$setY(0.11);
            return;
        }

        // Move up
        if (movingEntity.isTouchingWater() || movingEntity.isInLava()) {
            ((IVec3d) movingEntity.getVelocity()).wave$setY(0.11);
            ascending = 0;
            return;
        }
    ...
    private void onSendPacket(PacketEvent.Send event) {
        ...
        ((PlayerMoveC2SPacketAccessor) packet).wave$setY(packet.getY(0) - (0.02 + (0.0001 * swimmingTicks)));
    }
    ...
    private void onMoveEvent(PlayerMoveEvent event) {
        ...
        if (++swimmingTicks < 15) {
            if (mc.player.isOnGround()) {
                ...
                ((IVec3d) event.movement).wave$setXZ(vel.x, vel.y);
            }
        ...
        swimmingTicks = 0;
        if (slowDown.get()) ((IVec3d) event.movement).wave$setXZ(0, 0);
        ((IVec3d) mc.player.getVelocity()).wave$setY(0.08);
    }
