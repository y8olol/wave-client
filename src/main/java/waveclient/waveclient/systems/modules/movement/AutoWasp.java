...
public class AutoWasp extends Module {
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (avoidLanding.get()) {
            ...
            for (Direction dir : DirectionAccessor.wave$getHorizontal()) {
                BlockPos pos = BlockPos.ofFloored(targetPos.offset(dir, d).offset(dir.rotateYClockwise(), d)).down();
                if (((AbstractBlockAccessor) mc.world.getBlockState(pos).getBlock()).wave$isCollidable() && Math.abs(targetPos.getY() - (pos.getY() + 1)) <= 0.25) {
        ...
        ((IVec3d) event.movement).wave$set(xVel, yVel, zVel);
    }
