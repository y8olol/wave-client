...
import waveclient.waveclient.mixin.ClientPlayerEntityAccessor;
...
public class Flight extends Module {
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (offLeft <= 0 && delayLeft <= 0) {
            ...
            if (antiKickMode.get() == AntiKickMode.Packet) {
                ...
                ((ClientPlayerEntityAccessor) mc.player).wave$setTicksSinceLastPositionPacketSent(20);
            }
        } else if (delayLeft <= 0) {
    ...
    private void antiKickPacket(PlayerMoveC2SPacket packet, double currentY) {
        ...
        if (this.delayLeft <= 0 && this.lastPacketY != Double.MAX_VALUE &&
            ...
            ((PlayerMoveC2SPacketAccessor) packet).wave$setY(lastPacketY - 0.03130D);
        } else {
