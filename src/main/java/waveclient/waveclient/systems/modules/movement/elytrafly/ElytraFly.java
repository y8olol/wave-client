...
package waveclient.waveclient.systems.modules.movement.elytrafly;
...
import waveclient.waveclient.mixin.AbstractBlockAccessor;
...
import waveclient.waveclient.systems.modules.movement.elytrafly.modes.Bounce;
import waveclient.waveclient.systems.modules.movement.elytrafly.modes.Packet;
import waveclient.waveclient.systems.modules.movement.elytrafly.modes.Pitch40;
import waveclient.waveclient.systems.modules.movement.elytrafly.modes.Vanilla;
import waveclient.waveclient.systems.modules.player.ChestSwap;
import waveclient.waveclient.systems.modules.player.Rotation;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isGliding()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                    if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).wave$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3d) event.movement).wave$set(0, currentMode.velY, 0);
                }
            } else if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).wave$set(currentMode.velX, currentMode.velY, currentMode.velZ);
        ...
        if (autoHover.get() && mc.player.input.playerInput.sneak() && !Modules.get().get(Freecam.class).isActive() && mc.player.isGliding() && flightMode.get() != ElytraFlightModes.Bounce) {
            ...
            final boolean underCollidable = ((AbstractBlockAccessor) under).wave$isCollidable() || !underState.getFluidState().isEmpty();
            final boolean under2Collidable = ((AbstractBlockAccessor) under2).wave$isCollidable() || !under2State.getFluidState().isEmpty();

            if (!underCollidable && under2Collidable) {
                ((IVec3d)event.movement).wave$set(event.movement.x, -0.1f, event.movement.z);
            ...
            if (underCollidable) {
                ((IVec3d)event.movement).wave$set(event.movement.x, -0.03f, event.movement.z);
                ...
                if (mc.player.getEntityPos().y <= mc.player.getBlockPos().down().getY() + 1.34f) {
                    ((IVec3d)event.movement).wave$set(event.movement.x, 0, event.movement.z);
    ...
    protected void enableGroundListener() {
        WaveClient.EVENT_BUS.subscribe(staticGroundListener);
    }

    protected void disableGroundListener() {
        WaveClient.EVENT_BUS.unsubscribe(staticGroundListener);
    }
    ...
    protected void enableInstaDropListener() {
        WaveClient.EVENT_BUS.subscribe(staticInstadropListener);
    }

    protected void disableInstaDropListener() {
        WaveClient.EVENT_BUS.unsubscribe(staticInstadropListener);
    }
