...
public class Criticals extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof IPlayerInteractEntityC2SPacket packet && packet.wave$getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
            if (mace.get() && mc.player.getMainHandStack().getItem() instanceof MaceItem) {
                ...
                Entity entity = packet.wave$getEntity();
                    ...
                    case Jump, MiniJump -> {
                        if (!sendPackets) {
                            ...
                            if (mode.get() == Mode.Jump) {
                                ...
                                ((IVec3d) mc.player.getVelocity()).wave$setY(0.25);
                                sendTimer = 4;
                            }
                            event.cancel();
                        }
    ...
    private void sendPacket(double height) {
        ...
        ((IPlayerMoveC2SPacket) packet).wave$setTag(1337);
