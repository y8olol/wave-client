...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" ")) WaveClient.LOG.warn("Module '{}' contains invalid characters in its name making it incompatible with Wave Client commands.", name);
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) WaveClient.EVENT_BUS.subscribe(this);
                onActivate();
            }
        }
        else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) WaveClient.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
