...
import waveclient.waveclient.events.entity.player.InteractBlockEvent;
import waveclient.waveclient.events.entity.player.InteractEntityEvent;
...
import waveclient.waveclient.mixin.HandledScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((HandledScreenAccessor) screen).wave$getFocusedSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    WaveClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(ScreenHandler handler) {
        WaveExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        WaveExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
