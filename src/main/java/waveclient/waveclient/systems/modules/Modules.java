...
package waveclient.waveclient.systems.modules;
...
import waveclient.waveclient.systems.modules.combat.*;
import waveclient.waveclient.systems.modules.misc.*;
import waveclient.waveclient.systems.modules.misc.swarm.Swarm;
import waveclient.waveclient.systems.modules.movement.*;
import waveclient.waveclient.systems.modules.movement.elytrafly.ElytraFly;
import waveclient.waveclient.systems.modules.movement.speed.Speed;
import waveclient.waveclient.systems.modules.player.*;
import waveclient.waveclient.systems.modules.render.*;
import waveclient.waveclient.systems.modules.render.blockesp.BlockESP;
import waveclient.waveclient.systems.modules.render.marker.Marker;
import waveclient.waveclient.systems.modules.world.*;
...
import waveclient.waveclient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                WaveClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    /***
     * @see waveclient.waveclient.commands.commands.BindCommand
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        WaveClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    WaveClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    WaveClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
