...
package waveclient.waveclient.systems.hud.elements;

import waveclient.waveclient.systems.hud.Hud;
import waveclient.waveclient.systems.hud.HudElementInfo;

public class WaveTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", WaveTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {wave.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {wave.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{wave.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{wave.name} #1{wave.version}");
