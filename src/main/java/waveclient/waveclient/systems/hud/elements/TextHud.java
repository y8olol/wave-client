...
import waveclient.waveclient.gui.utils.StarscriptTextBoxRenderer;
...
public class TextHud extends HudElement {
    ...
    private void runTick(HudRenderer renderer) {
        ...
        try {
            if (script != null) {
                section = WaveStarscript.ss.run(script);
                calculateSize(renderer);
            }
        }
        ...
        if (shown.get() != Shown.Always && conditionScript != null) {
            String text = WaveStarscript.run(conditionScript);
