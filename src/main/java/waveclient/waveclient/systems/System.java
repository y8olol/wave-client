...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(WaveClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(WaveClient.MOD_ID, file.getName());
            ...
            WaveClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    WaveClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    WaveClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            WaveClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
